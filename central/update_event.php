...
try {
    ...
    $stmt = $pdo->prepare("SELECT id, image_path FROM events WHERE id = ?");
    ...
    $image_path = $existing_event['image_path']; // Keep existing image by default
    ...
    $stmt = $pdo->prepare("
        ...
        SET title = ?, description = ?, start_date = ?, end_date = ?, location = ?, 
            campus = ?, category = ?, cultural_groups = ?, image_path = ?, updated_at = CURRENT_TIMESTAMP
        WHERE id = ?
    ");
